...
class D2DSDK {
            ...
            when(payloadTransferUpdate.status){
                PayloadTransferUpdate.Status.SUCCESS -> {
                    ...
                    when(receivedPayload?.type){
                        ...
                        Payload.Type.FILE -> {}
                        Payload.Type.STREAM -> {
                            Log.e(TAG, "STREAM: ${payloadTransferUpdate.bytesTransferred} - ${payloadTransferUpdate.totalBytes}" )
                        }
                    }
                }
                PayloadTransferUpdate.Status.IN_PROGRESS -> {
                    if(listOfFilesIds.contains(payloadTransferUpdate.payloadId)) {
                        ...
                        if (!isAdvertiser) {
                            viewModel.relaySelectionTaskValue.value = JSONObject()
                                .put("type", "payloadTransferUpdate")
    ...
    fun launchSDK(owner: LifecycleOwner, listener: D2DListener?, context: Context){
        ...
        viewModel.relaySelectionTaskValue.observe(owner){ taskValue ->
            listener?.onReceivedTaskResul(D2D.ParameterTag.RELAY_SELECTION, taskValue)
        }
